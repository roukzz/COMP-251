...
public class DisjointSets {
    ...
    /*
     * merge sets containing elements i and j depending on the parent's rank of each
     */
    public int union(int i, int j) {
        // case when parent of i not the same as the parent of j
        if (find(i) != find(j)) {
            // merge i into j
            if (rank[find(i)] < rank[find(j)]) {
                par[find(i)] = find(j);
                return find(i);
                // when rank is equal merge i into j and increment the j parent's rank
