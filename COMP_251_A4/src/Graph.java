...
public class Graph {
    ...
    for (int i = 0; i < edges.size(); i++) {
      char node_1 = edges.get(i).node_1;
      char node_2 = edges.get(i).node_2;

      if (node_2 == u && node_1 != v) { // when last node is u, merge to v
        edges.get(i).node_2 = v; 
      }

      if (node_1 == u && node_2 != v) { // when first node is u, merge to v
        edges.get(i).node_1 = v; 
      }
    }

    for (int i = 0; i < edges.size(); i++) {
      char node_1 = edges.get(i).node_1;//get the first node
      char node_2 = edges.get(i).node_2;//get the second node

      if (node_2 == u && node_1 == v) { // remove the edge v-u
        edges.remove(i);
        i--;
      }

      if (node_2 == v && node_1 == u) { // remove the edge u-v
        edges.remove(i);
        i--;
      }
      if (node_1 == u && node_2 == u) { // when self loop
        edges.remove(i);
      }
    }
    removeNode(u); 
